...
//    goal == b.b1 && goal == b.b2
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val nexts = initial.map(ini => neighborsWithHistory(ini._1, ini._2)).flatten
      val nextsUnexplored = newNeighborsOnly(nexts, explored)
      nextsUnexplored #::: from(nextsUnexplored, explored ++ nexts.map(_._1))
    }
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    from(List((startBlock, List())).to(LazyList), Set(startBlock))
  ...
  lazy val solution: List[Move] =
    val results = pathsToGoal.map(_._2).toList
    if (results.isEmpty) List() else results.head.reverse
