...
trait GameDef:
  ...
  case class Block(b1: Pos, b2: Pos):
    ...
    def neighbors: List[(Block, Move)] =
      def applyMove(direction: Move) = direction match {
        case Move.Left => (left, Move.Left)
        case Move.Right => (right, Move.Right)
        case Move.Up => (up, Move.Up)
        case Move.Down => (down, Move.Down)
      }
      List(Move.Left, Move.Right, Move.Up, Move.Down).map(applyMove)
